...
class _HardScreenState extends State<HardScreen> {
  ...
  // Cargar la puntuación más alta del nivel difícil
  Future<void> _loadHighScore() async {
    int score = await ScoreManager.getHighScore('hard');
    setState(() {
      highScore = score;
    });
  }
  ...
  void _onGameComplete(int time) {
    ...
    if (finalTime < highScore || highScore == 0) {
      ScoreManager.saveHighScore('hard', finalTime);
      setState(() {
        highScore = finalTime;
      });
    }
